          ...
          success: function(data) {
                if( data[0].t != null &&  data[0].l_fix != null ){
                    ...
                    if( data[0].c.indexOf("+") > -1  ){
                        ...
                        arrowimage = '<img src="wp-content/themes/pin-wp/images/arrow_green.png" style="width:16px; margin-left: 37px;">';
                    }else{
                        string = "<span class='lastpriceh'>"+data[0].l_fix+"</span>&nbsp;&nbsp;<span class='percentageh'>"+stock_cp_fix+"%</span>";
                        class_hover = "tag-format-red-hover";
                        arrowimage = '<img src="wp-content/themes/pin-wp/images/arrow_red.png" style="width:16px; margin-left:37px;">';
                    }
...
(function($){
    ...
    $.fn.myajax = function(){
     $('div[name="trig"]').each(function(){ //input[class=tag-list]
            var self = this;
            var tags = $(this).find("input[class=tag-list]").val(); 
            var string;
                            ...
                            if( count < 2 )
                            {
                                ...
                                if( array[index].LastTradePriceOnly != null && array[index].PercentChange !=  null )
                                {
                                   ...
                                   // $(self).find('.tag-format').remove();
                                    ...
                                    $(self).append('<button style="top:-3.1em;margin-left:20px;" class="tag-format aboveclass '+class_hover+'" title="'+array[index].Name+'" >&nbsp;'+ array[index].symbol +" "+ string +'&nbsp;</button>');
                                    //console.log(self);
...
var posted_date = jQuery(this).find('.date-icon').val();
 if( tags.length > 0){ 
    ...
    var url = '//query.yahooapis.com/v1/public/yql?q=select * from yahoo.finance.quotes where symbol in ("'+array1[0]+'")&env=http://datatables.org/alltables.env&format=json';
    jQuery.get(url, function(data){
      var x = data['query']['results'].quote;
      diff = numberDiff(x.LastTradePriceOnly,array1[1]);
      var sign = diff > 0 ? 1 : -1;
