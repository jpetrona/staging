...
class RetirelyWebSocket
{
	...
	*/
	...
	function wsSendClientClose($clientID, $status=false) {
		...
		$userid = $this->wsClients[$clientID][12];
		$usertype = $this->wsClients[$clientID][13];
		$url = "http://retire.ly/mobile/function.php?method=SetOffline&id=".$userid."&usertype=".$usertype;		 
		$ch = curl_init();
	...
	function wsSetUserID($clientID, $userID, $usertype) {
		$this->wsClients[$clientID][12] = $userID;
		$this->wsClients[$clientID][13] = $usertype;
		$url = "http://retire.ly/mobile/function.php?method=SetOnline&id=".$userID."&usertype=".$usertype;		 
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
	...
	function wsSentNotification($userID, $usertype, $message, $custmsg)
	{
		$m = urlencode($message);
        $cm = urlencode($custmsg);
		$url = "http://retire.ly/mobile/function.php?method=SendNotification&id=".$userID."&usertype=".$usertype."&msg=".$m."&custmsg=".$cm;		 
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS,"id=".$userID."&usertype=".$usertype."&msg=".$m."&custmsg=".$cm);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
		curl_close($ch);		 
	}
	
	function wsSaveChatHistory($senderID, $receiverID, $sender, $message)
	{
              $m = urlencode($message);
		$url = "http://retire.ly/mobile/function.php?method=SaveChatHistory";		 
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS,"senderid=".$senderID."&receiverid=".$receiverID."&sender=".$sender."&message=".$m);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
		curl_close($ch);	 
	}
