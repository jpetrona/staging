...
$que = "INSERT INTO `users` SET `userid`='',`fname`='$fname',`lname`='$lname',`email`='$email',`fbid`='',`linkedinid`='',`password`='$pwd',`photopath`='$imagepath',`location`='$location',`age`=$age,`gender`='$gender',`occupation`='$occupation',`university`='$university',`phone`='$phone',`investableamout`=$amount,`lat`=$lat,`lng`=$lng,`devicetoken`='$devicetoken',`devicetype`='$devicetype',`online`=0,`date`='{$date}',`status`=10,`socialpageurl`=''";

        if(mysql_query($que) or die(mysql_error()))
        {
            $id = mysql_insert_id();
			...
			$resdetail=mysql_query($q) or die(mysql_error());	
			$rowdetail=mysql_fetch_assoc($resdetail);
			$rowdetail['fname'] = stripslashes($rowdetail["fname"]);
			$rowdetail['lname'] = stripslashes($rowdetail["lname"]);
			$rowdetail['email'] = stripslashes($rowdetail["email"]);
...
function AdvisorSignUp()
{
    ...
    if(mysql_num_rows($res)==0)
    {	
        $q = "INSERT INTO advisors VALUES ('','$fname','$lname','$email','','','$pwd','$imagepath','$location',$age,'$gender','$firm','$title','$university','$professionalbio','$phone','$website','$currentoffice',$lat,$lng,'$keywords','$devicetoken','$devicetype',0,'{$date}',10,0,0,'{$date}','','$aum','')";
        if(mysql_query($q))
        {
            $id = mysql_insert_id();
...
function SocialSignIn()
{
    ...
    if($usertype == "user")
	{
		...
		{
			$q = "SELECT * FROM users WHERE linkedinid = '$linkedinid'";
		}
		$res=mysql_query($q);	
		$row=mysql_fetch_row($res);	
		if(mysql_num_rows($res)==0)
		{	
			/*
			$que = "SELECT * FROM users WHERE devicetoken = '$devicetoken' AND devicetype = '$devicetype'";
			$res=mysql_query($que);	
			$row=mysql_fetch_row($res);	
			if(mysql_num_rows($res) > 0)
			{
			*/			
				$query = "UPDATE users SET devicetoken = '', devicetype = '' WHERE devicetoken = '$devicetoken' AND devicetype = '$devicetype'";        
	...
	'$devicetoken','$devicetype',0,'{$date}',10,'$socialpageurl')";
				...
				if(mysql_query($q))
				{
					...
					$q = "UPDATE users SET online = '1' WHERE userid = $id";        
					mysql_query($q);
					$contents['fname'] = stripslashes($fname); 
					$contents['lname'] = stripslashes($lname);
					$contents['email'] = stripslashes($email);
					$contents['photopath'] = $imagepath;
					$contents['location'] = $location;
					$contents['age'] = (string)$age;
		...
		else
		{
			...
			$q = "UPDATE users SET online = '1', devicetoken = '$devicetoken', devicetype = '$devicetype', 
			      lat = $lat, lng = $lng, socialpageurl = '$socialpageurl', photopath = '$imagepath' WHERE userid = $userid";        
             mysql_query($q);
			...
			$contents['fname'] = stripslashes($rowdetail["1"]);
			$contents['lname'] = stripslashes($rowdetail["2"]);
			$contents['email'] = stripslashes($rowdetail["3"]);
			$contents['photopath'] = $rowdetail["7"];
			$contents['location'] = $rowdetail["8"];
			$contents['age'] = (string)$rowdetail["9"];
	...
	else if($usertype == "advisor")
	{
		if($fbid != "")
		{
			$q = "SELECT * FROM advisors WHERE fbid = '$fbid'";
		}else if($linkedinid != "")
		{
			$q = "SELECT * FROM advisors WHERE linkedinid = '$linkedinid'";
		}
		$res=mysql_query($q);	
		$row=mysql_fetch_row($res);	
		if(mysql_num_rows($res)==0)
		{	
			/*
			$que = "SELECT * FROM advisors WHERE devicetoken = '$devicetoken' AND devicetype = '$devicetype'";
			$res=mysql_query($que);	
			$row=mysql_fetch_row($res);	
			if(mysql_num_rows($res) > 0)
			{
			*/			
				$query = "UPDATE advisors SET devicetoken = '', devicetype = '' WHERE devicetoken = '$devicetoken' AND devicetype = '$devicetype'";        
				mysql_query($query) or die(mysql_error());
				$q = "INSERT INTO advisors VALUES ('','$fname','$lname','$email','$fbid','$linkedinid','$pwd','$imagepath','$location',$age,'$gender','$firm','','','','','','',$lat,$lng,'','$devicetoken','$devicetype',0,'{$date}',10,0,0,'{$date}','$socialpageurl','$aum','')";
				if(mysql_query($q))
				{
					$contents = array(); 
					$id = mysql_insert_id();
		...
		{	
			...
			$q = "UPDATE advisors SET online = '1', devicetoken = '$devicetoken', devicetype = '$devicetype', 
			      lat=$lat, lng = $lng, socialpageurl = '$socialpageurl', photopath = '$imagepath' WHERE advisorid = $advisorid";        
            mysql_query($q);
...
function ChangePassword()
{
    ...
    if(isset($_POST["oldpassword"]))
    {
        ...
        $oldpassword =  mysql_real_escape_string(md5($oldpassword));
    }
    if(isset($_POST["newpassword"]))
    {
       $newpassword = $_POST["newpassword"];
       $newpassword = mysql_real_escape_string(md5($newpassword));
    }
    if(isset($_POST["usertype"]))
    {
       $usertype= $_POST["usertype"];
...
phone = '$phone', website = '$website', currentoffice = '$currentoffice', lat = $lat,lng = $lng, searchkeywords = '$keywords',aum = '$aum' 
...
function SearchAdvisors()
{
		...
		$q = "SELECT 
			...
			AND advisors.advisorid NOT IN(SELECT advisorid FROM chats WHERE userid = $userid) ";
		...
		{  
			...
			$q = $q." ORDER BY advisors.advisorid ";			
		}			
		...
		{		
			...
			while($advisor = mysql_fetch_assoc($res)) {
				$advisor["fname"] = stripslashes($advisor["fname"]);
				$lname = $advisor["lname"];
			    $advisor["lname"] = mb_convert_encoding($lname, "UTF-8", "HTML-ENTITIES");
...
function SearchUsers()
{
       //GetPurchaseStatus();
		...
		$q = "SELECT 
			...
			AND users.userid NOT IN(SELECT userid FROM chats WHERE advisorid = $advisorid) ";
       ...
       $sql = "Update advisors SET lat = $lat,lng = $lng  WHERE advisorid = $advisorid";
		...
		{		
			...
			while($user = mysql_fetch_assoc($res)) { 
				$user["fname"] = stripslashes($user["fname"]);
				$user["lname"] = stripslashes($user["lname"]);
		      ...
		      while($favorite = mysql_fetch_assoc($res)) {
			    ...
			    if(mysql_num_rows($res1) > 0)
			    {
					...
					while($review = mysql_fetch_assoc($res1)) {
						$review["fname"] = stripslashes($review["fname"]);
						$lname = $review["lname"];
			            $review["lname"] = mb_convert_encoding($lname, "UTF-8", "HTML-ENTITIES");						
						$reviews[] = array('review'=>$review );
					}				    
					$favorite["reviews"] = $reviews;
			    }
				$favorite["fname"] = stripslashes($favorite["fname"]);
				$lname = $favorite["lname"];
			    $favorite["lname"] = mb_convert_encoding($lname, "UTF-8", "HTML-ENTITIES");
				$favorite["email"] = stripslashes($favorite["email"]);
...
favorites.date as favoritedate,
		   ...
		   {	
		      while($favorite = mysql_fetch_assoc($res)) {
					$favorite["fname"] = stripslashes($favorite["fname"]);
					$favorite["lname"] = stripslashes($favorite["lname"]);
					$favorite["email"] = stripslashes($favorite["email"]);
      		        $favorites[] = array('favorite'=>$favorite);
    		      } 
                  }   
...
function ForgetPassword()
{
		...
		$ency_pwd = "";	
$usertype = "";
		$ency_pwd = mysql_real_escape_string(md5($pwd));
        if(isset($_POST["email"]))
        {
            $email = $_POST["email"];
            $email = mysql_real_escape_string($email);
...
if(mysql_num_rows($res1) > 0)
{
     while($review = mysql_fetch_assoc($res1)) {
		$review["fname"] = stripslashes($review["fname"]);
		$review["lname"] = stripslashes($review["lname"]);
        $reviews[] = array('review'=>$review );
     }				    			   
}
header('Content-type: application/json');     
...
{
$deviceToken = '36e3aee2423aa9f62038d2709f01196498ff90e07cf77155da223de984963166';
      ...
      while($chat = mysql_fetch_assoc($res1)) {
		...
		if($usertype == "user")
		{
			...
			if(mysql_num_rows($res2) > 0)
			{
				...
				$msg = explode("\n",$row2["message"]);
				$chat["message"] = stripslashes(end($msg));
				$chat["id"] = $row2["id"];
...
function SearchUsersTest()
{
        ...
        if($keywords == "worldwide")
		{
			...
			$q = $q." ORDER BY users.userid limit 500";
		}
...
function SocialSignIn1()
{
	...
	'$devicetoken','$devicetype',0,'{$date}',10,'$socialpageurl')";
		...
		else
		{
			$userid = $row["0"];
			$q = "UPDATE users SET online = '1', devicetoken = '$devicetoken', devicetype = '$devicetype', 
			      lat = $lat, lng = $lng, socialpageurl = '$socialpageurl', location = '$location', photopath = '$imagepath' WHERE userid = $userid";        
             mysql_query($q);
	...
	else if($usertype == "advisor")
	{
		...
		{	
			...
			//GetAdvisorPurchaseStatus($advisorid);
			$q = "UPDATE advisors SET online = '1', devicetoken = '$devicetoken', devicetype = '$devicetype', 
			      lat=$lat, lng = $lng, socialpageurl = '$socialpageurl', location = '$location',photopath = '$imagepath' WHERE advisorid = $advisorid";        
            mysql_query($q);
			$q = "SELECT * FROM advisors WHERE advisorid = $advisorid";
			$resdetail=mysql_query($q);	
			$rowdetail=mysql_fetch_row($resdetail);
