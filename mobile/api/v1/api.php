...
function SignIn()
{
    ...
    if(isset($_POST['devicetoken']))
    {
        ...
        $devicetoken = mysql_real_escape_string($devicetoken);
    }
...
function MarkAsFavorites()
	{
		...
		if(mysql_num_rows($res) == 0)
		{	
			$q= "INSERT INTO favorites VALUES ('',$userid,$advisorid,'$usertype','{$date}',10)";
			if(mysql_query($q))
			{
		   ...
		   $q = "SELECT 
			  ...
			  INNER JOIN advisors ON advisors.advisorid = favorites.advisorid
                          LEFT OUTER JOIN
                             (SELECT COUNT(*) AS reviewCount, advisorid FROM reviews GROUP BY advisorid) as reviewcounts
                          ON reviewcounts.advisorid = advisors.advisorid 
			  WHERE favorites.userid = $userid AND advisors.advisorid NOT IN(SELECT blockeduserid FROM blockedusers WHERE senderid = $userid AND usertype = 'user') ";	
		   $res = mysql_query($q);
		   if(mysql_num_rows($res) > 0)
		   {	
		      while($favorite = mysql_fetch_assoc($res)) {
                ...
                else if($advisorid > 0)
                {
                   ...
                   $q = "SELECT 
				         ...
				         users.socialpageurl,
                                         'user' As usertype,
						 favorites.favoriteid  FROM favorites 
                      INNER JOIN users ON users.userid = favorites.userid
                      WHERE favorites.advisorid = $advisorid AND users.userid NOT IN(SELECT blockeduserid FROM blockedusers WHERE senderid = $advisorid AND usertype = 'advisor') ";	
                   $res = mysql_query($q);
		   if(mysql_num_rows($res) > 0)
		   {	
		      while($favorite = mysql_fetch_assoc($res)) {
...
function ContactUs1()
{
        $email = 'chouhan.virendra@gmail.com';
