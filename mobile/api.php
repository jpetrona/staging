...
{  
	...
	if($usertype == "user")
	{
		$q = "SELECT chats.*,advisors.fname,
			...
			'advisor' AS usertype
			FROM chats
			INNER JOIN advisors ON advisors.advisorid = chats.advisorid	
			WHERE userid = $id";
	}
	else if($usertype == "advisor")
	{
		$q = "SELECT chats.*, users.fname,
			users.lname,
			users.email,
			users.photopath,
			users.location,
			...
			'user' AS usertype
			FROM chats 
			INNER JOIN users ON users.userid = chats.userid WHERE advisorid = $id";
	}
    $res1=mysql_query($q);	
    $chats = array();			
    if(mysql_num_rows($res1) > 0)
