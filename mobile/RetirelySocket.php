...
class RetirelyWebSocket
{
	...
	*/
	...
	function wsSendClientClose($clientID, $status=false) {
		...
		$url = "http://projects.linkites.com/retirely/function.php?method=SetOffline&id=".$userid."&usertype=".$usertype;		 
		$ch = curl_init();
	...
	function wsSetUserID($clientID, $userID, $usertype) {
		...
		$url = "http://projects.linkites.com/retirely/function.php?method=SetOnline&id=".$userID."&usertype=".$usertype;		 
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
	...
	function wsSentNotification($userID, $usertype, $message, $custmsg)
	{
		...
		$url = "http://projects.linkites.com/retirely/function.php?method=SendNotification&id=".$userID."&usertype=".$usertype."&msg=".$m."&custmsg=".$cm;		 
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
	...
	{	    
	    $m = urlencode($message);
		$url = "http://projects.linkites.com/retirely/function.php?method=SaveChatHistory&senderid=".$senderID."&receiverid=".$receiverID."&sender=".$sender."&message=".$m;		 
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
