...
function insert_fb_in_head() {
    ...
    if(!has_post_thumbnail( $post->ID )) { //the post does not have featured image, use a default image
        $default_image="//www.retire.ly/wp-content/themes/pin-wp/images/retire-logo-insta-3.svg"; //replace this with a default image on your server or an image in your media library
...
// New wp-editor Setup for front end post initilize ///
...
function feed_author($name) {
   ...
   // $tag_addition = "<input type='hidden' value='{$input}'><a class=' the_author_link' style='color:blue;border-bottom:none;' href='{$string}'>$name</a>";
    if($name == 'Larry Iser'){
        ...
        <a class='the_author_link' target='_blank' style='color:blue;border-bottom:none;' href='http://www.kwikalaw.com '>$name</a>";
    }else{
        $tag_addition = "<input type='hidden' value='{$input}'>
        <a class='the_author_link' style='color:blue;border-bottom:none;' href='{$string}'>$name</a>";
    }
    return $tag_addition;
}
...
function the_source($postid){
    ...
    if( $source != '' ){
        ...
        if(sizeof($result)>0){
            ...
            $img_source = "<a href='{$link}'><img src='{$link_for_source_images}' style='height:auto;max-width:80px;'/></a>";
            echo  $img_source;
        }
    }
}
   ...
   if($counter){
	   foreach($getRecords as $key=>$value){
		 $result['last_id'] = $value->chat_id;
	        $html .= "<div class='msg' title=".stripslashes($value->name)."><span class='chatername'>".stripslashes($value->name).":<span class='chat-datetime'>{$value->posted}</span></span><span class='msgc'>".stripslashes($value->msg)."</span></div>";
            }
	   $result['msgs'] = $html;
   }else{
	   $result['last_id'] = '';
