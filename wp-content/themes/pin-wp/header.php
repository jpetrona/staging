...
global $usernm;
?>
<meta name="google-site-verification" content="_FJbmDHOAIziNVJIx-cZLIrT5vykFKWGMlyayhupNpo" />
...
<header>
  <div class="main-header">
    ...
    <!--Donwload Btn link-->
     <div class="download-store-btn">
         <ul>
            ...
            <li><a href="https://play.google.com/store/apps/details?id=com.Intlfaces.retire.ly"><img src="<?php echo site_url();?>/wp-content/themes/pin-wp/images/google-play-white.svg" class="img-responsive social-img-icon"></a></li>
            <li><a href="https://itunes.apple.com/us/app/retirely/id713998164?ls=1&mt=8"><img src="<?php echo site_url();?>/wp-content/themes/pin-wp/images/apple-button-white.svg" class="img-responsive social-img-icon"></a></li>
          </ul>
        </div>
    <nav id="myjquerymenu" class="jquerycssmenu header-menu">
      ...
      <div class="header-SignInButton" <?php if(!is_user_logged_in()){?>style="margin-top:5px;" <?php } ?>> <?php echo do_shortcode("[SignInButton]"); ?> </div>
        </nav>
...
<div  class="featured-articles" style="height:159px;">
  ...
  <ul class="featured-articles-slider">
	...
	<input type="hidden" value="<?php echo (isset($value->source) && $value->source != '') ? $value->source  : ''  ; ?>"  class="expertcomment">
	...
	if(isset($post->post_author) && $post->post_author != 0) { 
	      ...
	      $auth_user_name = get_userdata($post->post_author)->user_login;
              $user_description = get_user_meta($post->post_author);
             ...
             //$user_description = $user_description['description'][0];
             if($user_description==""){
               //$user_description = get_user_meta($post->post_author)['Country'][0];
             }
        }else{
      ...
      ?>
        <?php
        $source_name = get_post_meta( $post->ID, 'source',true);
        if($source_name!=""){
          $source_sql = "SELECT * FROM a1_source WHERE sourceimg LIKE '%".$source_name."%'";
          $source_data_ar = $wpdb->get_results($source_sql, OBJECT);
          $source_name = $source_data_ar[0]->sourcename;
        }
        ?>
        <input type="hidden" class="expertprofileimage" value="<?php echo $avtar_img;?>">
	<input type="hidden" class="authusername" value="<?php echo $auth_user_name;?>">
        <input type="hidden" class="expertname" value="<?php echo $author_name;?>">
        <input type="hidden" value="<?php echo $source_name;?>"  class="expertcomment">
     ...
     <?php if ( has_post_thumbnail()) { ?>
      ...
      <div class="title-box frontend-title">
        <h2 class="stockname_heading"><a class="stockname" href="<?=$redirect_link?>">
          <?php if ( strlen(get_the_title()) > 33 ) { echo substr(get_the_title(), 0, 33)." ..."; } else { the_title(''); } ?>
          </a></h2>
        <?php
        global $post;
        ?>
...
?>
<input type="hidden" id="is_singlepage" value="<?php echo $issingle;?>">
