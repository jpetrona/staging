...
jQuery(document).ready(function(){
   jQuery(document).on("mouseover", ".jlk-graph-post", function(e){
       e.preventDefault(); 
    });
    /*Like on index page*/
        ...
        var post_id = jQuery(this).attr("data-post-id");
            jQuery.ajax({
               ...
               success: function(response) {
                    ...
                    jQuery(".like-unlike-popup-sec").html(response.post_users_list);
...
function record(url, result) {
   if(result == 'success'){
      jQuery('.retina').attr("src",url).show();  
    }else{
       jQuery('.retina').hide(); 
    }
}  
        jQuery('#fep-source-link').on("blur", function(){
          ...
          var checksource = $('#token-input-fep-source').val();
          ...
          jQuery.ajax({
              ...
              beforeSend: function() {
                 ...
                 $('#retina1').attr("src", "");
                 $('#token-input-fep-source').attr("value", "");
              },
              success: function(response) {
              if(response != false){
                $('#retina1').attr("src", response[0].sourceimg);
                $('#retina1').show();
                $('#token-input-fep-source').attr("value",response[0].sourcename);
                 }else{
    ...
    jQuery(".watch-action, .watch-position, .like-unlike-popup-sec").hover(
     function(){
        $(".like-unlike-popup-sec").show();
    },
    function(){
        $(".like-unlike-popup-sec").hide();
    }
    );
        ...
        onSelected: function (data) {
            ...
            if(window.pageload=="pageload"){
                ...
                if(!inputIndex){
                  ...
                  window.location = location.protocol + '//' + location.host+ '/?cat_id='+ val;
                }
            }
            else{
