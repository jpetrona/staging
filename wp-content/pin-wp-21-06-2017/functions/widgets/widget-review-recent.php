...
class anthemes_recentreviews extends WP_Widget {
    function anthemes_recentreviews() {
      $widget_ops = array('description' => esc_html__('Your site\'s Recent Reviews.', 'anthemes'));
      parent::WP_Widget(false, $name = ''. esc_html__('Custom: Recent Reviews', 'anthemes') .'',$widget_ops); 
    }
...
<?php $anposts = new WP_Query(array('meta_key' => 'taq_review_score', 'ignore_sticky_posts' => 1, 'orderby' => 'date', 'order' => 'DESC', 'posts_per_page' => esc_attr($number) )); // number to display more / less ?>
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("anthemes_recentreviews");')); // register widget
?>