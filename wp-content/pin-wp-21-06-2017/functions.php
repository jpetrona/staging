   ...
   if($counter){
       foreach($getRecords as $key=>$value){
         $result['last_id'] = $value->chat_id;
            $html .= "<div class='msg' title=".stripslashes($value->name).">".get_avatar($value->sender_id, 30)."<span class='chatername'>".stripslashes($value->name).":<span class='chat-datetime'>{$value->posted}</span></span><span class='msgc'>".stripslashes($value->msg)."</span></div>";
            }
       $result['msgs'] = $html;
   }else{
       $result['last_id'] = '';
...
// function setlikedislikeTime($post_id,$user_id){
//     global $wpdb;
...
function getPostsLikeDislikecount($post_id,$user_id){
   global $wpdb;
   $data = ''; 
   $sql  ="select count(*) as likecount from a1_postlike WHERE post_id = ".$post_id."";
...
function getSinglePostLikeDislikeUsers($post_id){
   ...
   $time_query = "select likedislike_time from a1_postlike WHERE post_id = ".$post_id."";
   $opt = $wpdb->get_results($sql);
   $opt2 = $wpdb->get_results($sql2);
   $likedislike_time = $wpdb->get_results($time_query);
   $data['like_users'] = $opt;
   $data['dislike_users'] = $opt2;

   return $data;
}
function getlikedislikeTime($post_id){
   global $wpdb;
   $data = ''; 
   $sql  ="select user_id from a1_postlike WHERE post_id = ".$post_id." and like_dislike=1";
   $sql2  ="select user_id from a1_postlike WHERE post_id = ".$post_id." and like_dislike=-1";
   ...
   $data['like_users'] = $opt;
   $data['dislike_users'] = $opt2;
   $data['likedislike_time'] = $likedislike_time;
