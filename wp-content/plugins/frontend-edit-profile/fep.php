...
/*Version Check*/
global $wp_version;
$exit_msg = "Please, upgrade your Wordpress Installation.";
...
class FRONTEND_EDIT_PROFILE{
	...
	function form_style() {
		...
		//wp_register_style('fep-tab-style','//code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css','','1.0');
		wp_register_style('fep-tab-style',get_template_directory_uri().'/css/jquery-ui.css','','1.0');
		wp_enqueue_style('fep-tab-style');
	...
	function form_script(){
		...
		$src = $plugins_url.'/'.FEP.'/fep.js';
 		//wp_enqueue_script( 'password-strength-meter' );
		
		//wp_enqueue_script('fep-tab1-script','//code.jquery.com/ui/1.11.4/jquery-ui.js','','1.0');
	...
	function process_form( $atts ){
		...
		update_user_meta( $user_id , 'nickname', $_REQUEST['last_name'] );
        ...
        if( !empty( $_POST['pass1'] ) && !empty( $_POST['pass2'] ) )
        {
            ...
            if ( !( $user && wp_check_password( $_POST['old-pass'] , $user->data->user_pass, $user_id ) ) )
            {
            	$style = "error";
            	$message = __("<strong>Failed Data Entry </strong>: Profile can not be updated");
        ...
        /************************************************/
         ...
         $private_messages = $_POST['private_messages'];
         update_user_meta( $user_id ,'private_messages', $private_messages , '' );
         $subscription = $_POST['subscription'];
         update_user_meta( $user_id ,'subscription', $subscription , '' );
         $AccountName = $_POST['AccountName'];
         update_user_meta( $user_id ,'AccountName', $AccountName , '' );
         $AccountName = $_POST['AccountName'];
         update_user_meta( $user_id ,'AccountName', $AccountName , '' );
         $demo = $_POST['demo'];
         update_user_meta( $user_id ,'demo', $demo , '' );
		...
		if(is_wp_error( $errors ) ) {
            ...
            {
                $message = __("<strong>Success</strong>:Profile Pic Deleted");
            }else if( isset( $_POST['old-pass'] ) && $_POST['old-pass'] != '' ){
                $message = __("<strong>Success</strong>:Profile Password Updated");
            }
            else {
