<?php
  // Original  code by Chirp Internet: .chirp..au
  ...
  class myRSSParser
  {
    ...
    function myRSSParser($file)
    {
      ...
      $data = preg_match("/^/", $file) ? file_get_contents($file) : file_get_contents($file);
      xml_parse($xml_parser, $data) or exit("Unable to connect");
    ...
    function startElement($parser, $tagname, $attrs=array())
    {
      ...
      if(preg_match("/^(RDF||CHANNEL|IMAGE|ITEM)/", $tagname)) {
        if($this->tags) {
