...
function fep_rollback(){
	...
	// 6/30/2015  edited for tags
	wp_deregister_style( 'fep-style-token' );
	wp_deregister_style( 'fep-style-token-facebook' );
	wp_deregister_style( 'fep-style-token-mac' );
	wp_deregister_script( 'fep-script-token' );
...
function fep_enqueue_files($posts) {
	...
	wp_enqueue_script( "form-jquery", 'https://ajax.googleapis.com/ajax/libs/jquery/2.1.3/jquery.min.js', array('jquery') );
       // wp_enqueue_script( "form-valid", get_template_directory_uri().'/js/jquery.validate.min.js', array('jquery') );
    	...
    	wp_enqueue_style( 'fep-style-token', plugins_url('static/css/token-input.css', __FILE__), array(), '1.0', 'all' );
        wp_enqueue_style( 'fep-style-token-facebook', plugins_url('static/css/token-input-facebook.css', __FILE__), array(), '1.0', 'all' );
        wp_enqueue_style( 'fep-style-token-mac', plugins_url('static/css/token-input-mac.css', __FILE__), array(), '1.0', 'all' );
        wp_enqueue_script( "fep-script-token", plugins_url( 'static/js/jquery.tokeninput.js', __FILE__ ), array('jquery') );
	...
	wp_enqueue_script( "typed-script", plugins_url( 'static/js/typed.js', __FILE__ ), array('jquery') );
